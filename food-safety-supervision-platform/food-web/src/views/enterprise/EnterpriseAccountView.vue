            ...
            <div class="hero-profile__body">
              ...
              <div class="hero-profile__meta">
                ...
                <span>
                  ...
                  {{ currentUser.username || enterpriseUser.username || "-" }}
                </span>
              </div>
            </div>
          </div>
        ...
        <section class="account-card">
          <div class="section-head">
            <div class="section-head__title">
              ...
              <div>
                <h2>基本信息</h2>
                <p>姓名和手机号支持自助维护，企业备案信息在专门的备案页面查看和维护。</p>
              </div>
            </div>
          ...
          <div class="info-panel">
            <div v-if="!editingBasic" class="field-grid">
              ...
              <article class="field-card">
                <span>登录账号</span>
                <strong>{{ currentUser.username || enterpriseUser.username || "-" }}</strong>
              </article>
              <article class="field-card">
                <span>账号状态</span>
            ...
            <div v-else class="field-grid">
              ...
              <label class="field">
                <span>关联企业</span>
                <input :value="enterpriseDisplayName" type="text" readonly />
              </label>
              <label class="field">
                <span>手机号</span>
                <input
              ...
              <label class="field">
                <span>登录账号</span>
                <input :value="currentUser.username || enterpriseUser.username || '-'" type="text" readonly />
              </label>
              <label class="field">
                <span>账号状态</span>
        ...
        <section class="account-card side-card">
          <div class="section-head section-head--stack">
            <div class="section-head__title">
              ...
              <div>
                <h2>企业备案摘要</h2>
                <p>当前页面仅展示备案摘要，不在此处复制备案填报表单。</p>
              </div>
            </div>
            <RouterLink
              class="section-link-btn"
              :to="{ name: profileLoaded ? 'enterprise-profile-detail' : 'enterprise-profile' }"
            >
              {{ profileLoaded ? "查看备案详情" : "完善企业备案" }}
            </RouterLink>
          </div>
          ...
          <div class="side-fields">
            ...
            <article class="side-field">
              <span>统一社会信用代码</span>
              <strong>{{ enterpriseProfile.creditCode || "-" }}</strong>
            </article>
            <article class="side-field">
              <span>食品经营许可证编号</span>
              <strong>{{ enterpriseProfile.licenseNo || "-" }}</strong>
            </article>
            <article class="side-field">
              <span>法定代表人</span>
              <strong>{{ enterpriseProfile.legalRepresentative || "-" }}</strong>
            </article>
            <article class="side-field">
              <span>负责人</span>
              <strong>{{ enterpriseProfile.principal || "-" }}</strong>
            </article>
            <article class="side-field">
              <span>负责人电话</span>
              <strong>{{ enterpriseProfile.principalPhone || "-" }}</strong>
            </article>
            <article class="side-field">
              <span>所在地址</span>
              <strong>{{ enterpriseAddress }}</strong>
            </article>
          </div>
        </section>
...
const enterpriseProfile = reactive({
  enterpriseName: "",
  creditCode: "",
  licenseNo: "",
  legalRepresentative: "",
...
const roleLabel = "企业账号";
const displayName = computed(() => currentUser.realName || enterpriseProfile.principal || enterpriseUser.value.username || "企业用户");
...
const approvalLabel = computed(() => getApprovalStatusLabel(profileLoaded.value, enterpriseProfile.approvalStatus));
const approvalTone = computed(() => getApprovalStatusTone(profileLoaded.value, enterpriseProfile.approvalStatus));
const enterpriseDisplayName = computed(() => enterpriseProfile.enterpriseName || "未关联企业备案");
const enterpriseAddress = computed(() => (
...
function fillCurrentUser(data) {
  ...
  currentUser.userType = data?.userType || "ENTERPRISE";
  currentUser.updateTime = data?.updateTime || "";
}

function fillProfile(data) {
  enterpriseProfile.enterpriseName = data?.enterpriseName || "";
  enterpriseProfile.creditCode = data?.creditCode || "";
  enterpriseProfile.licenseNo = data?.licenseNo || "";
  enterpriseProfile.legalRepresentative = data?.legalRepresentative || "";
  enterpriseProfile.principal = data?.principal || "";
  enterpriseProfile.principalPhone = data?.principalPhone || "";
  enterpriseProfile.addressDetail = data?.addressDetail || "";
  enterpriseProfile.regionPathText = data?.regionPathText || "";
  enterpriseProfile.approvalStatus = data?.approvalStatus || "";
  enterpriseProfile.createTime = data?.createTime || "";
}
