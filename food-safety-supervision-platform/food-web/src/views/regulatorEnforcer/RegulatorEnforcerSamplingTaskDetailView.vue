    ...
    <section class="sampling-detail-page">
      <header class="page-head">
        ...
        <div class="head-actions">
          ...
          <button v-if="task?.status === 'COMPLETED'" class="primary-btn primary-btn--secondary" type="button" :disabled="actionLoading" @click="handleCloseTask">
            归档任务
          </button>
        </div>
      </header>
...
import { closeSamplingTask, fetchOperationAuditLogs, findMySamplingTaskById } from "../../api/regulationOperation";
...
const timeline = computed(() => {
  ...
  return auditLogs.value.map((item, index) => ({
    ...
    desc: item.summary || item.remark || item.actionName || item.actionType || "抽检任务操作",
...
const planLines = computed(() => {
  ...
  if (current.samplingResult === "PASS") {
    return [
      "整理抽检材料并归档，纳入企业日常监管记录。",
      "如无额外处置需求，归档后可返回列表继续处理后续任务。"
    ];
  }
  return [
...
async function loadDetail() {
  ...
  try {
    ...
    if (route.query.submitted === "1") setStatus("抽检结果已提交", "success");
...
.primary-btn--secondary { background: #0f4ea5; border-color: #0f4ea5; }
...
.hero-grid { display: grid; grid-template-columns: minmax(0, 1.7fr) 360px; gap: 16px; }
.hero-card, .timeline-card, .panel, .time-card { border: 1px solid #dbe3ee; background: #fff; }
.hero-card { display: grid; grid-template-columns: 220px minmax(0, 1fr); gap: 18px; padding: 20px; }
.hero-card__media { position: relative; min-height: 220px; background: linear-gradient(135deg, #dfe8f5 0%, #f3f6fb 100%); overflow: hidden; }
.hero-card__tag { position: absolute; top: 12px; left: 12px; padding: 4px 8px; background: rgba(0, 38, 96, 0.86); color: #fff; font-size: 10px; font-weight: 800; letter-spacing: 0.08em; text-transform: uppercase; }
.hero-card__placeholder { height: 100%; display: grid; place-items: center; color: #335bae; font-size: 22px; font-weight: 900; letter-spacing: 0.08em; }
.hero-card__top { display: flex; justify-content: space-between; gap: 16px; }
.hero-card__chips { display: flex; gap: 8px; flex-wrap: wrap; }
.chip { display: inline-flex; align-items: center; min-height: 24px; padding: 0 10px; border-radius: 999px; font-size: 11px; font-weight: 800; }
.chip--primary { background: #dbeafe; color: #1e3a8a; }
.chip--plain { background: #eef2f7; color: #475569; }
.hero-card h2 { margin: 12px 0 0; color: #0f172a; font-size: 28px; font-weight: 900; }
.hero-card p { margin: 6px 0 0; color: #64748b; font-size: 13px; }
.hero-batch { text-align: right; }
.hero-batch span, .hero-metrics span, .timeline-card__head span, .panel-head span, .result-summary span, .assign-card span, .kv-list dt { display: block; color: #64748b; font-size: 11px; font-weight: 800; }
.hero-batch strong { display: block; margin-top: 6px; color: #002660; font-size: 16px; font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; }
.hero-metrics { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px; margin-top: 18px; padding-top: 18px; border-top: 1px solid #e8edf3; }
.hero-metrics article { padding: 12px; background: #f8fafc; }
.hero-metrics strong { display: block; margin-top: 6px; color: #0f172a; font-size: 14px; }
.timeline-card { padding: 18px; }
.timeline-card__head, .panel-head { display: flex; justify-content: space-between; gap: 10px; align-items: center; margin-bottom: 14px; }
.timeline-card__head h3, .panel-head h3 { margin: 0; color: #002660; font-size: 15px; font-weight: 900; }
.timeline-list { display: grid; gap: 16px; }
.timeline-item { position: relative; padding-left: 20px; }
.timeline-dot { position: absolute; left: 0; top: 5px; width: 10px; height: 10px; border-radius: 999px; }
.timeline-dot.is-on { background: #002660; }
.timeline-dot.is-alert { background: #ba1a1a; }
.timeline-dot.is-muted { background: #cbd5e1; }
.timeline-item strong { display: block; color: #0f172a; font-size: 13px; }
.timeline-item small { display: block; margin-top: 4px; color: #64748b; font-size: 11px; }
.timeline-item p, .plan-list li { margin: 6px 0 0; color: #475569; font-size: 12px; line-height: 1.6; }
.timeline-empty { color: #64748b; font-size: 12px; padding: 4px 0; }
.assign-card { margin-top: 18px; padding: 12px; background: #f8fafc; }
.assign-card p { margin: 6px 0 0; color: #334155; font-size: 12px; line-height: 1.6; }
.content-grid { display: grid; grid-template-columns: minmax(0, 1.6fr) 340px; gap: 16px; }
.main-column, .side-column { display: grid; gap: 16px; }
.panel { padding: 16px; }
.panel--soft { background: linear-gradient(180deg, #f5f9ff 0%, #edf4ff 100%); }
.result-summary { display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
.result-summary article { padding: 12px; background: #f8fafc; }
.result-summary .is-wide { grid-column: 1 / -1; }
.result-summary strong, .kv-list dd { display: block; margin-top: 6px; color: #0f172a; font-size: 13px; }
.time-card { display: flex; justify-content: space-between; align-items: center; gap: 12px; padding: 18px 16px; border-left: 5px solid #002660; background: #f6f9ff; }
.time-card strong { display: block; margin-top: 6px; color: #002660; font-size: 22px; font-weight: 900; }
.time-card__right { text-align: right; }
.time-card strong.warning { color: #b45309; }
.time-card strong.danger { color: #b91c1c; }
.plan-list { margin: 0; padding-left: 18px; }
.kv-list { display: grid; gap: 10px; margin: 0; }
.kv-list div { display: flex; justify-content: space-between; gap: 10px; }
.kv-list dd { margin: 0; text-align: right; }
.status-banner { padding: 10px 12px; border: 1px solid #dbe3ee; background: #f8fafc; color: #334155; font-size: 13px; }
