  ...
  >
    <section class="enterprise-admin-page">
      <header class="enterprise-admin-page__head">
        <div>
          <h1>企业名录管理</h1>
          <p>辖区内全量食品经营主体档案与监管状态库</p>
        </div>
        <div class="enterprise-admin-page__actions">
          <button type="button" class="ghost" @click="onPendingFeature('导出名单')">导出名单</button>
        </div>
      </header>
      ...
      <section class="enterprise-filter-grid">
        ...
        <div>
          ...
          <select v-model="filters.approvalStatus">
            ...
            <option value="APPROVED">已过</option>
        ...
        <table>
          ...
          <tbody v-if="records.length">
            <tr v-for="item in records" :key="item.id">
              ...
              <td><span class="badge" :class="{ key: item.status === 'KEY' }">{{ formatStatus(item.status) }}</span></td>
              <td><span class="badge" :class="approvalBadgeClass(item.approvalStatus)">{{ formatApprovalStatus(item.approvalStatus) }}</span></td>
              <td>{{ formatTime(item.updateTime) }}</td>
              <td><button type="button" class="link" @click="handleViewDetail(item)">查看详情</button></td>
            </tr>
          </tbody>
        </table>
        <div v-if="!records.length" class="empty">暂无企业数据</div>
      </section>
      ...
      <div class="stats-grid">
        <article><p>Active Entities</p><strong>{{ total }}</strong></article>
        <article><p>Key Regulatory</p><strong>{{ keyCount }}</strong></article>
        <article><p>Pending Review</p><strong>{{ pendingCount }}</strong></article>
        <article><p>Approved</p><strong>{{ approvedCount }}</strong></article>
      </div>
...
.enterprise-admin-page__actions { display: flex; gap: 10px; }
.enterprise-admin-page__actions button, .table-footer button, .enterprise-filter-grid__submit { border: 0; border-radius: 8px; padding: 9px 14px; cursor: pointer; font-size: 13px; }
.enterprise-admin-page__actions .ghost, .table-footer button { background: #e2e8f0; color: #1e293b; }
.enterprise-admin-page__actions .primary, .enterprise-filter-grid__submit { background: #002660; color: #fff; }
...
.enterprise-filter-grid__submit { align-self: end; min-width: 96px; }
