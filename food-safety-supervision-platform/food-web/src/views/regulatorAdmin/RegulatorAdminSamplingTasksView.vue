    ...
    <section class="sampling-page">
      ...
      <div class="filter-bento">
        <div class="filter-bento__main filter-panel">
          ...
          <div class="filter-panel__body">
            ...
            <p class="filter-hint">
              任务名称为前端筛选（当前页数据）；相关企业、执行状态由查询请求后端；右侧结果与公示仅筛选当前页已加载数据。
              <!-- TODO: 后端支持按任务标题关键字查询后可将任务名称改为服务端筛选。 -->
            </p>
          </div>
        </div>
        <div class="filter-bento__side">
          <div class="side-title">
            <span>结果与公示过滤</span>
            <span class="side-title-icon">≡</span>
          </div>
          <div class="side-block">
            ...
            <div class="pill-group">
              <button
                type="button"
                :class="{ active: uiFilters.resultFilter === 'PASS' }"
                @click="toggleResultFilter('PASS')"
              >
                合格
              </button>
              <button
                type="button"
                :class="{ active: uiFilters.resultFilter === 'FAIL' }"
                @click="toggleResultFilter('FAIL')"
              >
                不合格
              </button>
              <button
                type="button"
                :class="{ active: uiFilters.resultFilter === 'PENDING' }"
                @click="toggleResultFilter('PENDING')"
              >
                待处理
              </button>
            </div>
          ...
          <div class="side-block">
            ...
            <div class="pill-group">
              <button
                type="button"
                :class="{ active: uiFilters.publicFilter === 'DRAFT' }"
                @click="togglePublicFilter('DRAFT')"
              >
                草稿
              </button>
              <button
                type="button"
                :class="{ active: uiFilters.publicFilter === 'PUBLISHED' }"
                @click="togglePublicFilter('PUBLISHED')"
              >
                已公示
              </button>
              <button
                type="button"
                :class="{ active: uiFilters.publicFilter === 'OFFLINE' }"
                @click="togglePublicFilter('OFFLINE')"
              >
                已下线
              </button>
            </div>
            ...
            <tr v-for="task in displayedTasks" :key="task.id">
              ...
              <td class="td-right">
                <div class="action-row">
                  ...
                  <span
                    v-else-if="isSamplingTaskAssignable(task) && isTaskDeadlineExceeded(task.deadline)"
      ...
      <div class="table-footer">
        <span class="footer-meta">
          显示 {{ rangeLabel }}，共 {{ totalDisplay }} 条记录
        </span>
...
const displayedTasks = computed(() => {
  ...
  if (kw) {
    list = list.filter((t) => (t.taskTitle || t.productName || "").toLowerCase().includes(kw));
  }
  ...
  if (uiFilters.publicFilter) {
    if (uiFilters.publicFilter === "DRAFT") {
      list = list.filter((t) => !t.samplingPublicStatus || t.samplingPublicStatus === "DRAFT");
    } else {
      list = list.filter((t) => (t.samplingPublicStatus || "") === uiFilters.publicFilter);
    }
  }
  return list;
});
...
.filter-grid-3 input, .filter-grid-3 select {
...
.filter-grid-3 input:hover, .filter-grid-3 select:hover { border-color: #cbd5e1; }
.filter-grid-3 input:focus, .filter-grid-3 select:focus {
  outline: none;
  border-color: #002660;
  box-shadow: 0 0 0 3px rgba(0, 38, 96, 0.12);
}
.filter-grid-3 input::placeholder { color: #94a3b8; }
.filter-actions {
  display: flex;
  gap: 8px;
  flex-shrink: 0;
...
.pill-group button {
  border: 0; background: rgba(255, 255, 255, 0.14); color: #fff; font-size: 12px; font-weight: 700;
  padding: 8px 12px; border-radius: 6px; cursor: pointer; transition: background 0.15s; min-height: 36px;
}
.pill-group button:hover { background: rgba(255, 255, 255, 0.24); }
...
.table-footer {
  ...
  padding: 10px 12px; background: #fff; border: 1px solid #e2e8f0; border-radius: 8px; font-size: 10px; font-weight: 700; color: #64748b; text-transform: uppercase; letter-spacing: 0.04em;
}
...
.action-row {
  ...
  padding: 4px; border-radius: 8px; background: #f8fafc; border: 1px solid #e2e8f0;
}
.op-btn {
  border: 1px solid #cbd5e1; background: #fff; color: #334155; border-radius: 6px; padding: 5px 10px;
  min-height: 28px; font-size: 12px; font-weight: 700; line-height: 1; cursor: pointer; transition: all 0.15s ease;
}
...
.op-expired-tip {
  ...
  border: 1px solid #fecaca; background: #fef2f2; color: #b91c1c; font-size: 12px; font-weight: 700; white-space: nowrap;
}
