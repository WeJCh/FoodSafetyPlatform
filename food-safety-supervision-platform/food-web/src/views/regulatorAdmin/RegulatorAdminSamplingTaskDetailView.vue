        ...
        <div v-if="task.samplingResult === 'FAIL'" class="sd-risk" role="alert">
          <div class="sd-risk__left">
            ...
            <div>
              ...
              <p class="sd-risk__desc">
                任务“{{ task.taskTitle || task.productName || "-" }}”检出结论为不合格，请按流程启动核查与后续处置。
              </p>
            </div>
          </div>
          ...
          <div class="sd-col sd-col--aside">
            <section class="sd-card">
              ...
              <div class="sd-mini-grid">
                <div class="sd-mini">
                  <span class="sd-mini__label">任务编号</span>
                  <span class="sd-mini__val mono">{{ task.taskNo || "-" }}</span>
                </div>
                <div class="sd-mini">
                  <span class="sd-mini__label">品类</span>
                  <span class="sd-mini__val">{{ task.productCategory || "-" }}</span>
                </div>
              </div>
              ...
              <dl class="sd-kv">
                <div class="sd-kv__row">
                  <dt>受检企业</dt>
                  <dd>{{ enterprise?.enterpriseName || task.enterpriseName || "-" }}</dd>
                </div>
                <div class="sd-kv__row sd-kv__row--time">
                  <dt>采样 / 截止时间</dt>
                  <dd>{{ formatTime(task.sampledTime) }} · 截止 {{ formatTime(task.deadline) }}</dd>
                </div>
                <div class="sd-kv__row">
                  <dt>执行人员</dt>
            ...
            <section class="sd-card">
              ...
              <div v-if="timeline.length" class="sd-timeline">
                <div v-for="(ev, idx) in timeline" :key="ev.key || idx" class="sd-timeline__item">
                  ...
                  <div>
                    <div class="sd-timeline__title">{{ ev.title }}</div>
                    <div class="sd-timeline__sub">{{ ev.sub }}</div>
                  </div>
                </div>
              </div>
          ...
          <div class="sd-col sd-col--main">
            <section class="sd-pub">
              <div class="sd-pub__text">
                <div class="sd-pub__badge">
                  <span class="sd-pub__pulse" />
                  公示状态：{{ formatSamplingPublicStatus(task.samplingPublicStatus) }}
                </div>
                <h3 class="sd-pub__title">公示管理控制台</h3>
                <p class="sd-pub__desc">
                  当前检测结果对公众披露状态如上。可在下方查看实验室结论摘要，并决定是否发布或下线公示信息。
                </p>
              </div>
              <div class="sd-pub__actions">
                <button
                  ...
                  type="button"
                  class="sd-btn sd-btn--on-dark sd-btn--ghost"
                ...
                <button
                  v-if="task.samplingResultId && task.samplingPublicStatus !== 'PUBLISHED'"
                  type="button"
                  class="sd-btn sd-btn--on-dark sd-btn--accent"
                  :disabled="actionLoading"
                  @click="handlePublish"
                >
                  立即发布
            ...
            <section class="sd-card sd-card--table">
              <div class="sd-table-head">
                ...
                <div
                  v-if="task.samplingResult"
                  class="sd-table-flag"
                  :class="{ 'is-fail': task.samplingResult === 'FAIL' }"
                >
                  <span class="sd-table-flag__dot" />
                  {{ task.samplingResult === "FAIL" ? "检测到不合格项" : "未检出异常" }}
                </div>
                <div v-else class="sd-table-flag is-pending">
                  <span class="sd-table-flag__dot" />
                  待执法人员提交结果
                </div>
              </div>
                ...
                <table class="sd-table">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td class="sd-col-result sd-col-result__cell">
                        <span class="sd-pill" :class="resultPillClass(task.samplingResult)">
              ...
              <div class="sd-lab-notes">
                <div class="sd-note-block">
                  <span class="sd-note-block__label">任务与产品说明</span>
                  <p class="sd-note-block__text">{{ task.taskDesc || "暂无补充说明。" }}</p>
                </div>
              </div>
            </section>
            ...
            <section class="sd-split">
              <div class="sd-card sd-card--accent2 sd-card--full">
                ...
                <ul class="sd-plan">
                  <li v-for="(line, i) in planLines" :key="i">{{ line }}</li>
                </ul>
              </div>
            </section>
          </div>
...
const timeline = computed(() => {
  ...
  return auditLogs.value.map((item, index) => ({
    ...
    sub: `${formatTime(item.createTime)} · ${formatLogOperatorName(item.operatorName)}`,
...
const planLines = computed(() => {
  ...
  if (current.samplingResult === "FAIL") {
    return [
      `通知“${current.enterpriseName || enterprise.value?.enterpriseName || "企业"}”限期整改并留存书面记录。`,
      "视情况启动批次追溯与同类企业排查。",
      "公示前请复核结论与证据材料完整性。"
    ];
  }
...
async function loadDetail() {
  ...
  try {
    ...
    auditLogs.value = [...taskLogs, ...resultLogs].sort(
...
async function handlePublish() {
  ...
  setStatus("");
  try {
    await publishSamplingResult(token.value, current.samplingResultId);
    setStatus("抽检结果已公示", "success");
    await loadDetail();
  } catch (e) {
    setStatus(e?.message || "公示失败", "error");
  } finally {
...
async function handleOffline() {
  const current = task.value;
  if (!current?.samplingResultId) return;
  actionLoading.value = true;
  setStatus("");
  try {
    await offlineSamplingResult(token.value, current.samplingResultId);
    setStatus("抽检结果已下线", "success");
    await loadDetail();
  } catch (e) {
    setStatus(e?.message || "下线失败", "error");
  } finally {
    actionLoading.value = false;
  }
}
...
function goList() {
  router.push({ name: "regulator-admin-sampling" }).catch(() => {});
}
...
.sd-title { margin: 0; font-size: 28px; font-weight: 900; letter-spacing: 0; color: #002660; }
