...
import { assignInspectionTask, closeInspectionTask, fetchInspectionTasks } from "../../api/regulationOperation";
...
const uiFilters = reactive({ deadlineStart: "", deadlineEnd: "", assigneeKeyword: "" });
const enforcerMap = reactive({});
const taskAssignments = reactive({});

const inProgressCount = computed(() => allDispatchTasks.value.filter((item) => item.status === "IN_PROGRESS").length);
const overdueCount = computed(() =>
  allDispatchTasks.value.filter((item) => isTaskDeadlineExceeded(item.deadline) && item.status !== "COMPLETED" && item.status !== "CLOSED").length
);
const completionRate = computed(() => {
  if (!dispatchTotal.value) return 0;
  const completed = allDispatchTasks.value.filter((item) => item.status === "COMPLETED" || item.status === "CLOSED").length;
...
function taskProgress(task) {
  const map = { CREATED: 0, ASSIGNED: 5, IN_PROGRESS: 40, COMPLETED: 100, CLOSED: 100 };
...
function taskMatchesUiFilters(task) {
  ...
  if (assigneeKeyword) {
    ...
    if (!assigneeName.includes(assigneeKeyword)) return false;
  }
  ...
  if (startDate && deadline < startDate) return false;
  if (endDate && deadline > endDate) return false;
  return true;
}
...
async function applyDispatchResult(tasks) {
  ...
  if (dispatchPage.value > dispatchPages.value) dispatchPage.value = dispatchPages.value;
...
async function loadDispatchTasks() {
  ...
  try {
    ...
    setStatus(resolveErrorMessage(error, "加载任务列表失败"), "error");
  } finally {
...
async function handleAssignTask(task) {
  ...
  try {
    ...
    setStatus(resolveErrorMessage(error, "任务派发失败"), "error");
  } finally {
    dispatchTaskLoading.value = false;
  }
}
...
async function handleCloseTask(task) {
  ...
  try {
    ...
    setStatus(resolveErrorMessage(error, "关闭任务失败"), "error");
  } finally {
    dispatchTaskLoading.value = false;
  }
}
...
.dispatch-subnav button { min-height: 30px; border-radius: 4px; border: 1px solid #d7e1ec; background: #fff; color: #516377; font-size: 12px; font-weight: 700; padding: 0 12px; cursor: pointer; }
...
.action-row { display: flex; gap: 6px; align-items: center; flex-wrap: wrap; padding: 4px; border-radius: 8px; background: #f8fafc; border: 1px solid #e2e8f0; }
